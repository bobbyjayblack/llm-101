...
try{
  ...
  await page.locator('#unit-select option').last().waitFor({state:'attached'});
  ...
  assert.equal(await page.locator('#unit-select option').count(),13);
  for(let i=0;i<lessons.length;i++){
    const lesson=lessons[i];
    if(await page.locator('#unit-select').inputValue()!==String(lesson.unit))await page.locator('#unit-select').selectOption(String(lesson.unit));
  ...
  await page.locator('#previous').click();assert.equal(await page.locator('#unit-select').inputValue(),'12');
  await page.locator('#next').click();assert.equal(await page.locator('#unit-select').inputValue(),'13');
  ...
  await page.locator('#unit-select').focus();await page.keyboard.press('Home');await page.keyboard.press('Enter');
  assert.equal(await page.locator('#unit-select').inputValue(),'1');
  ...
  await page.locator('#unit-select').selectOption('5');await page.locator('#run-lab').click();await page.locator('#unit-lab').scrollIntoViewIfNeeded();
