...
test('equations are placed beside existing paragraphs with spoken definitions',()=>{
  assert.equal(lessons.flatMap(l=>l.equations||[]).length,28);
  for(const lesson of lessons)for(const equation of lesson.equations||[]){
    assert.ok(Number.isInteger(equation.after)&&equation.after>=0&&equation.after<lesson.paragraphs.length);
    assert.ok(equation.title&&equation.expression&&equation.spoken.length>50);
  }
});
