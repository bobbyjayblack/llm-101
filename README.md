...
Each unit includes two lab scenarios, instructions, expected results, troubleshooting, and readable source. Predict before selecting **Run lab**; compare the result with the explanation and use the lesson's oral criteria to assess your understanding. Lab instructions, results, explanations, and recall prompts have read-aloud controls. Queries in units 9 and 13 use a small authored collection with lexical and fixed-vector ranking; unknown questions can return no evidence.

The curriculum now makes the research-to-engineering path inspectable. Unit 5 exposes token IDs, subword and unknown-token choices, next-token alignment, embedding lookup, and a tensor-level causal decoder trace. Data and evaluation lessons include contamination, n-gram overlap, subgroup slices, and calibration reporting. Adaptation compares supervised, low-rank, and preference objectives with a reward-hacking example. Retrieval labs show lexical and fixed-vector rankings, evidence reranking, source spans, version eligibility, provenance, and abstention. Serving worksheets separate calculated memory and throughput proxies from local measurements. Reliability labs include prompt-injection, malformed-tool, unsupported-citation, canary, and rollback fixtures. Lesson guides expose prerequisites, suggested time, readings, and success checks; the first six lessons retain authored recall prompts while existing notes and bookmarks remain compatible.

Units 9 and 13 still use a bounded, local extractive assistant. Fixed-vector retrieval and reranking are an optional Track A extension with the same evidence and refusal checks. A local generative adapter or conversational tutor remains a separately scoped future project and is not required for course completion.
